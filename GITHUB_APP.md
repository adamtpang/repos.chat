...
3. A human runs `repos approve` with the exact proposal ID. This sends the bounded request to the recipient Rep.
...
6. A human reviews that plan and runs `repos-github open` with the exact plan ID. The adapter rechecks every file hash, obtains a short-lived installation token, creates an app-authored commit, and opens a draft PR.
