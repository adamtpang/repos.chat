...
function readJsonFiles(dir, limit = 100) {
  let files = [];
  try {
...
function readMessages() {
  ...
  for (const inbox of inboxes) {
    ...
    let names = [];
    try { names = fs.readdirSync(path.join(mailRoot, inbox.name)).filter(name => name.endsWith('.json')); } catch {}
    for (const name of names) candidates.push(path.join(mailRoot, inbox.name, name));
  }
  const messages = [];
  for (const file of candidates.sort((a, b) => path.basename(b).localeCompare(path.basename(a))).slice(0, 100)) {
    try {
...
function readProposals() {
  return readJsonFiles(path.join(root, '.repo-connect', 'proposals'), 100)
    .map(proposal => ({
...
.stage{margin:0 0 18px;padding:14px;overflow:hidden}.stagehead,.panelhead{display:flex;justify-content:space-between;align-items:baseline;gap:12px;margin-bottom:10px}.stage h2,.panel h2{margin:0;font:800 18px "Segoe UI",sans-serif}.stagehead span,.panelhead span{color:var(--muted);font-size:13px}.mapwrap{overflow:auto;border:1px solid #303744;background:#0b1016}.mapwrap>svg{display:block;min-width:1000px;width:100%;height:auto}.edge{stroke:#5d6877;stroke-width:1;opacity:.18;stroke-dasharray:3 4}.edge.ready{stroke:var(--lime);stroke-width:1.7;opacity:.48;stroke-dasharray:none}.cluster-label{fill:#6e7885;font:700 9px Consolas,monospace;letter-spacing:.08em;text-transform:uppercase}.pet-label{fill:#d9d2bb;font:7px Consolas,monospace}.map-pet{cursor:pointer;outline:none}.map-pet>svg{width:16px;height:16px}.map-pet:hover .pet-label,.map-pet:focus .pet-label{fill:var(--lime)}.map-pet:focus .pet-target{stroke:var(--blue);stroke-width:2}.legend{display:flex;gap:16px;margin-top:8px;color:var(--muted);font-size:13px}.legend i{display:inline-block;width:18px;height:2px;margin-right:5px;vertical-align:middle;background:#5d6877}.legend i.ready{background:var(--lime)}
...
.purpose,.now span,.connections,.item strong,.gitrepo,.protocol .step{font-size:16px}.item small{font-size:13px}.now b,.gitrepo h3{font-size:16px}.meta,.tag,.badge,.petname,.route,.branch,.commit,.footer-note{font-size:12px}@keyframes bob{50%{transform:translateY(-3px)}}@keyframes blink{50%{opacity:.3}}@media(prefers-reduced-motion:reduce){.pet svg,.live i{animation:none}}@media(max-width:1100px){.cards{grid-template-columns:repeat(2,1fr)}.stats{grid-template-columns:repeat(4,1fr)}}@media(max-width:850px){.hero,.workbench{grid-template-columns:1fr}.cards{grid-template-columns:repeat(2,1fr)}}@media(max-width:580px){main{padding-inline:12px}header{align-items:flex-start;flex-direction:column}.rules,.cards,.cards.focused{grid-template-columns:1fr}.stats{grid-template-columns:repeat(2,1fr)}.protocol{grid-template-columns:repeat(2,1fr)}}
...
<section class="stage"><div class="stagehead"><h2>Repository connection map</h2><span>solid lime = executable recipe · gray dash = related only · select a pet to focus</span></div><div class="mapwrap" id="map"></div><div class="legend"><span><i class="ready"></i>ready to exchange work</span><span><i></i>relationship needs a recipe</span></div></section>
...
<section class="protocol"><div class="step"><b>1. Signal</b>manual / webhook / CI / drift</div><div class="step"><b>2. Proposal</b>local, not delivered</div><div class="step"><b>3. Approval</b>content-bound confirmation</div><div class="step"><b>4. Work</b>one locked recipient</div><div class="step"><b>5. Draft PR</b>app-authored, tested</div><div class="step"><b>6. Human merge</b>authority stays human</div></section><p class="footer-note">This inspector binds to loopback. Repository names, messages, paths, and workspace metadata are never part of the public landing page.</p>
</main><dialog id="detail" aria-labelledby="detail-title"><button class="close" aria-label="Close">×</button><h2 id="detail-title">Durable record</h2><pre></pre></dialog>
<script>
...
function safeGithubUrl(value){try{const u=new URL(String(value));return u.protocol==='https:'&&u.hostname==='github.com'?u.href:null}catch{return null}}
...
function render(d){document.querySelector('#updated').textContent='habitat refreshed '+new Date(d.generatedAt).toLocaleTimeString();const f=document.querySelector('#focus');if(d.focus.length){f.className='focus visible';f.innerHTML='<b>Focused habitat:</b> '+d.focus.map(esc).join(' ↔ ')+' · <a href="/" style="color:var(--lime)">show the whole fleet</a>'}else{f.className='focus';f.textContent=''}const labels=[['repositories','pets'],['assigned','hatched'],['proactive','guarding'],['working','questing'],['pendingProposals','proposals'],['readyConnections','ready routes'],['conversations','threads']];document.querySelector('#stats').innerHTML=labels.map(([k,l])=>'<div class="stat"><b>'+d.summary[k]+'</b><span>'+l+'</span></div>').join('');document.querySelector('#map').innerHTML=layoutNetwork(d.network);document.querySelectorAll('.map-pet').forEach(p=>{const go=()=>location.href='?focus='+encodeURIComponent(p.dataset.id);p.onclick=go;p.onkeydown=e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();go()}}});const nodes=[...d.nodes].sort((a,b)=>Number(b.presence.proactive)-Number(a.presence.proactive)||a.id.localeCompare(b.id)),edgesBy=new Map;for(const edge of d.edges){if(!edgesBy.has(edge.from))edgesBy.set(edge.from,[]);edgesBy.get(edge.from).push(edge)}const floor=document.querySelector('#nodes');floor.className='cards'+(d.focus.length?' focused':'');floor.innerHTML=nodes.map(n=>card(n,edgesBy.get(n.id)||[])).join('');document.querySelector('#messages').innerHTML=records(d);document.querySelector('#git').innerHTML=gitFeed(nodes);document.querySelectorAll('.route-button').forEach(b=>b.onclick=()=>{document.querySelector('#detail pre').textContent=JSON.stringify(d.messages[Number(b.dataset.i)],null,2);document.querySelector('#detail').showModal()})}
const focusParam=new URLSearchParams(location.search).get('focus')||'',stateUrl='/api/state'+(focusParam?'?focus='+encodeURIComponent(focusParam):'');async function refresh(){if(document.querySelector('#detail').open||document.activeElement!==document.body)return;try{const response=await fetch(stateUrl,{cache:'no-store'});if(!response.ok)throw new Error('snapshot unavailable');render(await response.json())}catch(e){document.querySelector('#updated').textContent='habitat unavailable'}}refresh();setInterval(refresh,10000);document.querySelector('.close').onclick=()=>document.querySelector('#detail').close();
</script></body></html>`;

const snapshotCache = new Map();
async function refreshSnapshot(focus) {
  const normalized = normalizeFocus(focus).join(',');
  const childArgs = [currentFile, '--snapshot', '--root', root, '--depth', depth];
  if (normalized) childArgs.push('--focus', normalized);
  ...
  snapshotCache.set(normalized, { value, refreshedAt: Date.now(), pending: null });
  while (snapshotCache.size > 20) snapshotCache.delete(snapshotCache.keys().next().value);
  return value;
}

async function cachedSnapshot(focus) {
  const key = normalizeFocus(focus).join(',');
  const entry = snapshotCache.get(key);
  if (entry?.value && Date.now() - entry.refreshedAt < 5000) return entry.value;
  if (entry?.value) {
    if (!entry.pending) {
      entry.pending = refreshSnapshot(key).catch(() => null).finally(() => { entry.pending = null; });
    }
    return entry.value;
  }
  if (entry?.pending) return entry.pending;
  const pending = refreshSnapshot(key);
  snapshotCache.set(key, { value: null, refreshedAt: 0, pending });
  return pending;
}
...
const server = http.createServer(async (request, response) => {
  ...
  if (requestUrl.pathname === '/api/state') {
    try {
      const value = await cachedSnapshot(requestUrl.searchParams.get('focus') || '');
      response.writeHead(200, { ...headers, 'Content-Type': 'application/json; charset=utf-8' });
      response.end(`${JSON.stringify(value)}\n`);
    } catch (error) {
      if (!response.headersSent) response.writeHead(500, { ...headers, 'Content-Type': 'application/json; charset=utf-8' });
